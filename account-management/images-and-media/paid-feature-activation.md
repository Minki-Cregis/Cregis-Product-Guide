...
In addition to subscribing to a team plan, Cregis also supports feature-based expansion. Below are the individually purchasable features and the activation steps.

## 1. Increase Wallet Count

Expanding the number of wallets allows users to flexibly manage different assets separately, enabling better organization and control of various cryptocurrencies. This is particularly useful for enterprises or individuals managing large digital asset portfolios.
...
Apply to list new tokens on the platform. Once a token is successfully listed, only the team that submitted the application can trade that token.

Your current team plan includes a number of free listing attempts. These attempts do not expire—meaning renewing your plan will not grant additional free attempts. Once your free attempts are used up, each additional token listing application costs $350.
...
Each team plan comes with a monthly withdrawal limit that resets each month. Collection transactions and team account top-ups do not count toward this limit. Withdrawals that exceed the limit are subject to outbound transfer overage charge based on the excess amount. Businesses can expand their withdrawal limit according to their transaction volume to reduce excess service fees.
...
This feature allows clients—such as exchanges—to increase the maximum number of wallet addresses that can be created via API. As your user base grows, you may need to generate more wallet addresses to accommodate new users and transactions. Raising this limit enables your business to scale without being restricted by address creation caps.
...
Adding more automated risk management policies allows users to implement complex financial rules—such as approval workflows—improving the efficiency and accuracy of managing large asset portfolios while reducing manual intervention.
...
Automating the collection and signature process saves time, reduces human error, and makes asset management more efficient and secure. This feature is particularly valuable for businesses handling high transaction volumes, ensuring faster and more reliable operations.

If your current team plan does not include this feature, you can activate it for an additional $500 per month.
