...
2.  一个关键步骤是将您的电商平台的伺服器 IP 地址添加到 Cregis 入口网站的 IP 白名单中。如果您的伺服器 IP 地址是静态，您可以在 Cregis Payment 设定中复制您的 IP 地址。然后在 Cregis 客户端导航到 设置 > IP白名单，添加您的伺服器 IP 地址，并保存更改。如果没有正确的 IP 白名单设置，来自您商店的 API 请求将被拒绝。
...
1.  登录您的 WordPress 管理后台，导航到 插件 > 添加插件。搜索「Cregis Payment for WooCommerce」并查找由 cregisproduct 创建的插件。点击 立即安装，然后点击 启用 完成安装。

    <figure><img src="../.gitbook/assets/image (1061).png" alt=""><figcaption></figcaption></figure>



2.  安装后，您将在左侧菜单中看到 Cregis Payment。点击访问配置页面。您需要从 Cregis 客户端复制 3 个信息：基础 URL、项目 ID 和 API 密钥。这些都可以在 Cregis 客户端的设置中找到。在插件中贴上这些值，然后点击 储存设定。<br>
...
3. 设置所需信息后，您的电商平台上销售的产品将可自动接受加密货币支付。<br>
   1.  订单金额基于您的 WooCommerce 商店货币设置，您可以在 WooCommerce > 设置 > 币种选项 > 货币 找到。（请确保您的货币是我们支持的货币，您可以在[此處](https://developers.cregis.com/zh/payment-engine-supported-currencies/)查阅）

       <figure><img src="../.gitbook/assets/image (1065).png" alt=""><figcaption></figcaption></figure>


   2.  产品价格是每个产品单独设置，置于 产品 > 全部产品 > \[选择产品] > 促销价格。
   ...
   6.  在 Web3 支付中，经常会遇到超额支付或部分支付的情况。您可以选择「接受所有不足额付款」来接受所有少付款，并选择「接受所有超额付款」来接受所有多付款。或者，您可以在插件设置中使用百分比配置容忍差，以控制您接受的上限和下限。例如，如果您在 100 美元的订单上设置 2% 的少额支付容差和 5% 的超额支付容差，您将接受 98 美元到 105 美元之间的支付。
...
1.  在您电商平台的结帐页面中，支付选项中将提供一个名为「Cregis Payment」的选项。付款人可以选择此选项并点击 下单。请注意，如果在库存中配置了产品数量（产品 > 所有产品 > 选择一个产品 > 产品数据 部分 > 库存 > 数量），当付款人点击「下单」按钮时，它将根据购物车中的数量自动扣除。
